...
$role_actions =RoleAction::where('role_id',Auth::user()->roles)->first(); ?>
    ...
    <?php $i++; ?>
                                ...
                                <?php if($role_id == 0){ ?>
                                <td><a href="{{ URL::Route('AdminReviewsDeleteWalker', $reviewp->review_id) }}"><input type="button" class="btn btn-success" value="Delete"></a></td>
                            <?php }else{?>
                             <?php 
                                $sub_action = SubRoleAction::where('role_id',Auth::user()->roles)->first();
                                    $review_action = $sub_action->review;
                                ?>
                                <?php if (strpos($review_action, '1') !== false) {
                                ?>
                                       <td><a href="{{ URL::Route('AdminReviewsDeleteWalker', $reviewp->review_id) }}"><input type="button" class="btn btn-success" value="Delete"></a></td>
                            <?php }?>
                ...
                <?php } ?>
                        ...
                        <?php foreach ($user_reviews as $reviewu) { ?>
                            <tr>
                                ...
                                <td >
                                    <?php echo $reviewu->created_at; ?>
                                <td><?= $reviewu->comment ?></td>
                                
                                <?php if($role_id == 0){ ?>
                                <td><a href="{{ URL::Route('AdminReviewsDeleteClient', $reviewu->review_id) }}"><input type="button" class="btn btn-success" value="Delete"></a></td>
                               <?php }else{?>
