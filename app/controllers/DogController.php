				...
				if (is_token_active($owner_data->token_expiry) || $is_admin) {
					...
					if($is_multiple_service->value == 0){
					    ...
					    $now = time();
	                   	...
	                   	$query = "SELECT walker.id, walker.first_name, walker.last_name, walker.picture, walker.phone, walker.latitude, walker.longitude, 1.609344 * 3956 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) as distance, ($now - last_name) as inactive_time from walker where is_available = 1 and is_active = 1 and is_approved = 1 and inactive_time < $inactive_timeout and (1.609344 * 3956 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) <= $distance order by distance";
